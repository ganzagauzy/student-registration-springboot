...
public class StudentCrsController {
    ...
    public ResponseEntity<List<StudentCourse>> listByCourse(@PathVariable Integer studentId){

        StudentRegistration stud = regService.getRegistrationById(studentId);

        if (studentId != null) {
            List<StudentCourse> crs = studCrsService.getCoursesByStudent(stud);
